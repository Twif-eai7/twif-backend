...
const BODY_CONTENT = [
  ...
  { type: 'para', text: 'The Parties shall first attempt to resolve disputes amicably. Failing resolution within thirty (30) days, disputes shall be referred to arbitration in Singapore under the Arbitration Rules of the Singapore International Arbitration Centre (SIAC). Seat of arbitration: Singapore. Language: English. The arbitration award shall be final and binding.' },

  { type: 'heading', text: '14. GOVERNING LAW' },
  { type: 'para', text: 'This Agreement shall be governed by the laws of Singapore. Courts of Singapore shall have exclusive jurisdiction for interim and enforcement proceedings.' },
  ...
  { type: 'para', text: 'This Schedule forms an integral part of the Master Non-Disclosure, Non-Circumvention & Non-Solicitation Agreement entered into between Twif Technologies PTE LTD ("Twif") and the Vendor.' },
...
async function buildSignedNdaPdf({ organization, ownerJobTitle, adminSignature, activityLog }) {
  ...
  const subtitle = `${vendorName} <> Twif Technologies PTE LTD`
  ...
  flow.text('Twif Technologies PTE LTD, having its headquarters in Singapore and a hub office in Bengaluru, India (hereinafter referred to as "Twif", which expression shall include its successors, affiliates and permitted assigns);', { font: fonts.serif, size: 10.5 })
  flow.spacer(6)
  ...
  await drawSignatureColumn(doc, flow.page, fonts, {
    ...
    label: 'For Twif Technologies PTE LTD, through its authorized signatory',
    ...
    company: 'Twif Technologies PTE LTD',
  ...
  allPages.forEach((page, i) => {
    ...
    page.drawText('Twif Technologies PTE LTD', { x: MARGIN, y: 28, size: 8, font: fonts.regular, color: INK_SOFT })
